...
if (!seal.ext.find("AI-role")) {
  ...
  // === 核心防卡死熔断器 ===
  async function safeFetchWithTimeout(url, options, timeoutMs = 180000) {
    let timerId;
  ...
  // === 新增：主线故事总结配置 ===
  seal.ext.registerBoolConfig(ext, "开启主线总结", false);
  seal.ext.registerStringConfig(ext, "主线总结前缀", "请根据最新对话与原有主线提取并更新当前剧情主线，仅返回<story_arc>[已经发生过的剧情主线]:具体的总结内容</story_arc>：\n");
  ...
  seal.ext.registerStringConfig(ext, "虚构角色历史", "system: \nuser: \nassistant: ");
  seal.ext.registerStringConfig(ext, "后置虚构角色历史", "user: \nassistant: ");
  seal.ext.registerStringConfig(ext, "虚构历史记录", "system: \nuser: \nassistant: ");
  seal.ext.registerStringConfig(ext, "后置虚构历史记录", "user: \nassistant: ");
  ...
  class DynamicConfig {
    ...
    refresh() {
      ...
      // 主线剧情载入
      this.enableStoryArc = seal.ext.getBoolConfig(this.ext, "开启主线总结");
      this.storyArcPrefix = seal.ext.getStringConfig(this.ext, "主线总结前缀");
  ...
  class ChatGPTSession {
    ...
    buildPayload() {
      ...
      // === 主线剧情定位注入点：系统提示之后，知识库 RAG 之前 ===
      const currentStoryArc = pConfig.storyArcAnchor || "";
      if (currentStoryArc) {
          coreMsgs.push({ role: "system", content: currentStoryArc, _type: "story_arc_context" });
      }
  ...
  async function updateStoryArc(session, aiReply, dynConfig) {
      const pConfig = session.personalConfig || {};
      const enableStoryArc = (pConfig.enableStoryArc !== null && pConfig.enableStoryArc !== undefined) 
          ? pConfig.enableStoryArc 
          : dynConfig.enableStoryArc;
  ...
  function rollbackAnchors(session, dynConfig) {
      ...
      for (let i = session.dynamicContent.length - 1; i >= 0; i--) {
          if (session.dynamicContent[i].role === "assistant") {
              ...
              if (!foundArc && session.dynamicContent[i].storyArcSnapshot !== undefined) {
                  lastValidArc = session.dynamicContent[i].storyArcSnapshot;
                  foundArc = true;
              }
              if (foundAnchor && foundArc) break;
          }
      }
  ...
  async function fetchImageToBase64(url) {
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        const response = await safeFetchWithTimeout(targetUrl, {}, 180000); 
...
身份识别码: ${formatFilterId(p.filterIdEnabled)}`;
      ...
      const toggleCommands = [
        ...
        { on: "开启主线总结", off: "关闭主线总结", key: "enableStoryArc", label: "主线总结" },
...
"文风总结": "enableStyleSummary", "主线总结": "enableStoryArc",
        ...
        if (keyToReset) {
            ...
            seal.replyToSender(ctx, msg, `✧ 未找到配置项「${target}」  \n支持重置：系统提示/api端点/api密钥/识别图片/联网请求/外部模组地址/模型名称/随机种子/纯净模式/温度/深度/主线总结等 或直接发送"重置配置"重置所有  `);
        }
        return true;
      }
      return false;
    ...
    try {
      ...
      if (loadModuleMatch) {
          ...
          if (!moduleName) {
              ...
              try {
                  ...
                  const response = await safeFetchWithTimeout(fileUrl, {}, 180000);
      ...
      if (/^继续生成$/i.test(text)) {
          ...
          try {
              ...
              updateSession(sessionKey, session);
              await Promise.all([
                  updateStatusBar(session, lastMsg.content, dynamicConfig),
                  updateStoryArc(session, lastMsg.content, dynamicConfig)
              ]);
          } catch (error) {
...
await Promise.all([
    ...
    updateStyleSummary(session, dynamicConfig)
              ...
              await Promise.all([
                  ...
                  updateStoryArc(session, result.originalReply, dynamicConfig)
              ]);
          } catch (error) {
